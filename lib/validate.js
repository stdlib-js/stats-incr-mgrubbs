...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0dt2h', options ) );
	}
	if ( hasOwnProp( options, 'alpha' ) ) {
		opts.alpha = options.alpha;
		if ( !isNumber( opts.alpha ) || isnan( opts.alpha ) ) {
			return new TypeError( format( '0dt8h', 'alpha', opts.alpha ) );
		}
	...
	if ( hasOwnProp( options, 'alternative' ) ) {
		...
		if ( !isString( opts.alternative ) ) {
			return new TypeError( format( '0dt2i', 'alternative', opts.alternative ) );
		}
	}
	return null;
}
