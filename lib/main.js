...
function incrmgrubbs( W ) {
	...
	if ( !isPositiveInteger( W ) ) {
		throw new TypeError( format( '0dtAD', W ) );
	}
	if ( W < 3 ) {
		throw new RangeError( format( '0dtAE', W ) );
	}
	opts = defaults();
	...
	*/
	function print( options ) {
		...
		if ( arguments.length > 0 ) {
			if ( !isObject( options ) ) {
				throw new TypeError( format( '0dt4K', options ) );
			}
			if ( hasOwnProp( options, 'digits' ) ) {
				if ( !isPositiveInteger( options.digits ) ) {
					throw new TypeError( format( '0dt3b', 'digits', options.digits ) );
				}
				digits = options.digits;
			}
			if ( hasOwnProp( options, 'decision' ) ) {
				if ( !isBoolean( options.decision ) ) {
					throw new TypeError( format( '0dt30', 'decision', options.decision ) );
				}
