...
function incrmgrubbs( W ) {
	...
	if ( !isPositiveInteger( W ) ) {
		throw new TypeError( format( '1Hz9b,Hn', W ) );
	}
	if ( W < 3 ) {
		throw new RangeError( format( '1Hz9c,Ho', W ) );
	}
	opts = defaults();
	...
	*/
	function print( options ) {
		...
		if ( arguments.length > 0 ) {
			if ( !isObject( options ) ) {
				throw new TypeError( format( '1Hz47,Gz', options ) );
			}
			if ( hasOwnProp( options, 'digits' ) ) {
				if ( !isPositiveInteger( options.digits ) ) {
					throw new TypeError( format( '1Hz3P,Fv', 'digits', options.digits ) );
				}
				digits = options.digits;
			}
			if ( hasOwnProp( options, 'decision' ) ) {
				if ( !isBoolean( options.decision ) ) {
					throw new TypeError( format( '1Hz2o,GE', 'decision', options.decision ) );
				}
